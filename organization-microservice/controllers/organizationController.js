...
// View Organization with Zapier Key
const viewOrganizationFromOtherService = asyncHandler(async (req, res) => {
  const organizationId = req.user.organization_id;
  const isAssociatedNumberRequired = req.headers["x-associated-number"]
  try {
    ...
    // if there is request for associated number too
    if (isAssociatedNumberRequired) {
      ...
      if (associatedNumber) {
        organization.associated_number = associatedNumber.plivo_number
      }
    }
...
const deleteOrganization = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    await PlivoRentedNumber.findOneAndUpdate(
      { organization_id: organizationId },
      {
        $set: {
          plivo_number: "",
...
const getOrganizationInfo = asyncHandler(async (req, res) => {
  const organizationId = req.params.organizationId;
  try {
    const organization = await Organization.findById(organizationId).select('emr_appointment_url');
    
    if (organization && organization.emr_appointment_url) {
      return res.status(200).json({ message: "EMR appointment URL", data: organization.emr_appointment_url });
    } else {
      return res.status(404).json({ message: "No EMR appointment URL found for the organization." });
    }
