...
async function verifyAuthenticUser(req, res, next, scope) {
  ...
  try {
    ...
    if (!permission || !Array.isArray(permission.permissions) || !permission.permissions.includes(scope)) {
      return res.status(401).json({ message: "Permission Denied" });
    }
...
// Has permission to access Dashboard
export const authenticUserToAccessDashboard = (req, res, next) => {
  const scope = scopeMaps["/dashboard"];
